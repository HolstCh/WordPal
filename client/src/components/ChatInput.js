...
export default function ChatInput({ inputText, setInputText, handleButtonClick }) {
    ...
    return (
        <div className="w-2/3 p-4 rounded-3xl border-2 border-gray-200 flex flex-row items-center">
            <TextareaAutosize
                minRows={1}
                maxRows={10}
                value={inputText}
                onChange={(e) => setInputText(e.target.value)}
                className="w-full mr-2 p-2 overflow-hidden rounded-3xl bg-gray-100 resize-none border-2"
            />
